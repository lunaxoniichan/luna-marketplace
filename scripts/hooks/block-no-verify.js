#!/usr/bin/env node
/**
 * PreToolUse Hook (Bash only): Block git hook-bypass flags.
 *
 * Blocks --no-verify and core.hooksPath overrides on git commit/push/merge/etc.
 * Vendored from ECC block-no-verify.js (flag-position-aware parser).
 *
 * Always-on security hook — intentionally NOT gated by LUNA_HOOK_PROFILE.
 * Disable only by removing the hook from hooks.json.
 *
 * Exit codes:
...
/**
 * Exportable run() for tests and direct invocation.
 */
